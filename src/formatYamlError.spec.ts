import z from 'zod'
import { formatYamlError } from './formatYamlError'
  ...
  describe('Options Handling', () => {
    it('should respect `useColor` option', () => {
      ...
      if (!result.success) {
        ...
        const errorWithColor = formatYamlError(issue, payload, {
          useColor: true,
          syntaxHighlighting: false,
        })
        const errorWithoutColor = formatYamlError(issue, payload, {
          useColor: false,
          syntaxHighlighting: false,
        })
    ...
    it('should respect `syntaxHighlighting` option', () => {
      ...
      if (!result.success) {
        ...
        const errorWithHighlighting = formatYamlError(issue, payload, {
          useColor: false,
          syntaxHighlighting: true,
        })
        const errorWithoutHighlighting = formatYamlError(issue, payload, {
          useColor: false,
          syntaxHighlighting: false,
        })
  ...
  describe('Edge Cases', () => {
    it('should throw error if path does not exist in payload', () => {
      ...
      if (!result.success) {
        ...
        expect(() =>
          formatYamlError(issue, invalidPayload, {
            useColor: false,
            syntaxHighlighting: false,
          }),
        ).toThrow(`Could not find position for path: ${issue.path.join('.')}`)
      } else {
        throw new Error('Validation should have failed')
      }
    })
